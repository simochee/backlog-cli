...
- ~~**`packages/cli/src/commands/auth/refresh.ts`**: OAuth トークンリフレッシュが未実装（行34-37）~~ **→ 解決済み**: `auth refresh` コマンドを実装し、`auth login -m oauth` も追加
...
| TBD | OAuth 2.0 ログイン（`auth login -m oauth`）＋トークンリフレッシュ（`auth refresh`）の実装 |
